...
def load_ackermann_2015():
    ...
    data_ackermann = np.loadtxt("/Users/brandonkelpis/Desktop/Carnegie/Paper_Code/data/apj504089t3_mrt.txt", skiprows=61, dtype="str")
...
def extragalactic_dI_dloge(E_GeV, cusp_dis, cusps):
    ...
    def Ngce(E_GeV): # this is the normalization function...... energy per second.....karwin is plotting photons per second.... how do i change this normalization s.t energy per second, its giving photons per second.
        return (8.6e14 * (E_GeV)**(0.27 - 0.27*np.log(E_GeV))) / (E_GeV *1000)  # out units Mev/s........ divide by E in MeV to get photons per second..
...
def intragalactic_dI_dloge(E_GeV, Jalphatot):
    ...
    """
    def Ngce(E_GeV):
        return (8.6e14 * (E_GeV)**(0.27 - 0.27*np.log(E_GeV))) / (E_GeV *1000)  # out units Mev/s
        # return 8.6e14 * (E_GeV)**(0.27 - 0.27*np.log(E_GeV)) 
